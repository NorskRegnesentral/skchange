...
def anomalies_to_labels(n: int, p: int, anomalies: List[Tuple[int, int]]) -> np.ndarray:
...
def format_multivariate_anomaly_output(
    ...
    if labels == "int_label":
        if fmt == "dense":
            anomaly_labels = anomalies_to_labels(n, p, anomalies)
            out = pd.DataFrame(
...
def format_anomaly_output(
    ...
    if labels == "int_label":
        if fmt == "dense":
            anomaly_labels = anomalies_to_labels(n, anomalies)
