...
def _compute_pinv_cdf_cov(X: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    """Compute the pseudo-inverse of the CDF covariance matrix.

    Parameters
    ----------
    data_ranks : np.ndarray
        The data ranks.
    ...
    """
    sample_size, n_variables = X.shape
    ...
    cdf_values = np.copy(data_ranks) / sample_size
    centered_cdf_values = cdf_values - 0.5

    cdf_cov = 4.0 * (centered_cdf_values.T @ centered_cdf_values) / sample_size
    ...
    return pinv_cdf_cov, data_ranks
