...
from skchange.costs._empirical_distribution_cost import (
    ...
    direct_empirical_distribution_cost,
...
def test_difficult_case():
    ...
    left_costs = direct_empirical_distribution_cost(
        data, left_intervals[:, 0], left_intervals[:, 1]
    )
    right_costs = direct_empirical_distribution_cost(
        data, right_intervals[:, 0], right_intervals[:, 1]
    )
    no_change_costs = direct_empirical_distribution_cost(
        data, full_intervals[:, 0], full_intervals[:, 1]
    )

    change_scores = no_change_costs - (left_costs + right_costs)
    print(f"Change scores for difficult case: {change_scores}")
    assert np.all(change_scores >= 0), (
        "Change scores should be non-negatve in this case."
    )
