...
def test_scorer_evaluate(Scorer: type[BaseIntervalScorer]):
    ...
    x.index = pd.date_range(start="2020-01-01", periods=x.shape[0], freq="D")
    scorer.fit(x)
    cut1 = np.linspace(0, 10, scorer.get_required_cut_size(), dtype=int)
    ...
    cut2 = np.linspace(10, 20, scorer.get_required_cut_size(), dtype=int)
...
def test_scorer_invalid_cuts(Scorer: BaseIntervalScorer):
    ...
    with pytest.raises(ValueError):
        cut = np.linspace(0, 10, scorer.get_required_cut_size(), dtype=float)
        scorer.evaluate(cut)
    with pytest.raises(ValueError):
        cut = np.linspace(0, 10, scorer.get_required_cut_size() - 1, dtype=int)
        scorer.evaluate(cut)
    with pytest.raises(ValueError):
        cut = np.linspace(0, 10, scorer.get_required_cut_size() + 1, dtype=int)
        scorer.evaluate(cut)
    with pytest.raises(ValueError):
        cut = np.linspace(10, 0, scorer.get_required_cut_size(), dtype=int)
        scorer.evaluate(cut)
