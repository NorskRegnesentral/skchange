...
def test_min_size_not_fitted():
    """Test that min_size raises NotFittedError when MultivariateTCost is not fitted."""
...
def test_nan_dof_raises_value_error():
    """Test that constructing MvTCost with nan fixed_dof raises ValueError."""
    with pytest.raises(ValueError):
        MultivariateTCost(fixed_dof=np.nan).fit(np.random.randn(100, 5))
