"""Tests for L2Saving with valued baseline parameters."""
...
def test_l2_saving_stores_scalar_baseline():
    """A scalar baseline_mean is stored as a 1-D array after fit."""
    scorer = L2Saving(baseline_mean=BASELINE_MEAN)
    X = make_no_change_X(scorer, loc=BASELINE_MEAN)
    scorer.fit(X)
    np.testing.assert_array_equal(scorer.baseline_mean_, [BASELINE_MEAN])


def test_l2_saving_broadcasts_to_n_features():
    """A scalar baseline_mean is broadcast to all n_features after fit."""
    scorer = L2Saving(baseline_mean=5.0)
    X = make_no_change_X(scorer, n_features=3, loc=5.0)
    scorer.fit(X)
    np.testing.assert_array_equal(scorer.baseline_mean_, [5.0, 5.0, 5.0])
...
def test_capa_l2_saving_finds_no_changepoint():
    """CAPA with a matched L2 valued baseline finds no changepoints."""
    scorer = L2Saving(baseline_mean=BASELINE_MEAN)
    X = make_no_change_X(scorer, loc=BASELINE_MEAN)
...
def test_capa_l2_saving_finds_single_changepoint():
    """CAPA with a matched L2 valued baseline detects the true changepoint."""
    scorer = L2Saving(baseline_mean=BASELINE_MEAN)
